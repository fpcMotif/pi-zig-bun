...
class SearchProcess {
  ...
  private nextId = 1;
  private buffer = "";
  private pending = new Map<number, { resolve: (v: any) => void; reject: (err: Error) => void }>();
...
async function ensureFixture(dir: string): Promise<void> {
  ...
  const run = spawnSync("bun", ["benchmarks/generate-fixture.ts", dir], { stdio: "inherit", cwd: process.cwd() });
  if (run.status !== 0) {
...
function benchmarkCliColdStart(samples: number): MetricSummary {
  ...
  for (let i = 0; i < samples; i++) {
    ...
    const run = spawnSync("bun", ["src/main.ts", "--help"], {
      cwd: process.cwd(),
      stdio: "ignore",
