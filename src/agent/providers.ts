...
export class AnthropicAdapter extends BaseSseAgent {
  ...
  protected parseChunk(payload: unknown) {
    ...
    if (p?.type === "content_block_delta" && p?.delta?.type === "text_delta") {
      return { token: p.delta.text };
    }
    if (p?.type === "content_block_start" && p?.content_block?.type === "tool_use") {
      return {
        toolCall: {
          id: p.content_block.id,
          name: p.content_block.name ?? "tool",
          arguments: JSON.stringify(p.content_block.input ?? {}),
        },
      };
    }
    return { done: p?.type === "message_stop" };
  }
}
...
export class GoogleGenAIAdapter extends BaseSseAgent {
  ...
  protected parseChunk(payload: unknown) {
    const p = payload as Record<string, any>;
    const text = p?.candidates?.[0]?.content?.parts?.map((part: Record<string, any>) => part.text ?? "").join("") ?? "";
